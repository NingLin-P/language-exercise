{-# LANGUAGE RankNTypes #-}

module Haskell.Codewars.Church where

import Prelude hiding (succ, pred, fst, snd)

pred :: Number -> Number
pred n = if v <= 0 then zero else churchify (v - 1)
  where v = eval n

subt :: Number -> Number -> Number
subt a b = if v >= 0 then churchify v else zero
  where v = eval a - eval b

churchify :: Integer -> Number
churchify 0 = zero
churchify n = succ $ churchify (n - 1)

-- cannot be placed in Preloaded right now as there is a bug in the codewars system

newtype Pair a b = Pr (forall c . (a -> b -> c) -> c)

instance (Show a, Show b) => Show (Pair a b) where
  show (Pr p) = p (\ a b -> "(" ++ show a ++ "," ++ show b ++ ")")

pair :: a -> b -> Pair a b
pair f s = Pr (\ b -> b f s)

fst :: Pair a b -> a
fst (Pr p) = p (\ a _ -> a)

snd :: Pair a b -> b
snd (Pr p) = p (\ _ b -> b)

newtype Number = Nr (forall a. (a -> a) -> a -> a)

instance Show Number where
  show (Nr a) = a ("1+" ++) "0"
  
instance Eq Number where
  a == b = eval a == eval b
  
fold :: Number -> (a -> a) -> a -> a
fold (Nr n) s z = n s z

eval :: Number -> Integer
eval (Nr a) = a (+1) 0

zero :: Number
zero = Nr (\ _ z -> z)

succ :: Number -> Number
succ (Nr a) = Nr (\ s z -> s (a s z))

add :: Number -> Number -> Number
add (Nr a) = a succ              

mult :: Number -> Number -> Number
mult (Nr a) b =  a (add b) zero