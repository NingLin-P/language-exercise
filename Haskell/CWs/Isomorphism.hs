...
isoUnMaybe (jab, jba) = (ab, ba)
  where ab a = fromMaybe (fromJust $ jab Nothing) (jab $ Just a)
        ba b = fromMaybe (fromJust $ jba Nothing) (jba $ Just b)
...
plusAssoc = (ab, ba)
  where ab (Left (Left a)) = Left a
        ab (Left (Right b)) = Right $ Left b
        ab (Right c) = Right $ Right c
        ba (Left a) = Left $ Left a
        ba (Right (Left b)) = Left $ Right b
        ba (Right (Right c)) = Right c
...
multAssoc = (ab, ba)
  where ab ((a, b), c) = (a, (b, c))
        ba (a, (b, c)) = ((a, b), c)

-- dist :: a * (b + c) = a * b + a * c
dist :: ISO (a, (Either b c)) (Either (a, b) (a, c))
dist = (ab, ba)
  where ab (a, Left b) = Left (a,b)
        ab (a, Right c) = Right (a,c)
        ba (Left (a, b)) = (a, Left b)
        ba (Right (a, c)) = (a, Right c)

-- (c ^ b) ^ a = c ^ (a * b)
curryISO :: ISO (a -> b -> c) ((a, b) -> c)
curryISO = (ab, ba)
  where ab g (a, b) = g a b
        ba g a b = g (a, b) 
...
-- error "do powSO" --_ `trans` powS `trans` _
-- Here's a trick: 
