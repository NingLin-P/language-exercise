...
class Functor f => Pretty f where
  render :: Pretty a => f (Expr a) -> String

pretty :: Pretty f => Expr f -> String
pretty (In a) = render a

instance Pretty Lit where
  render (Lit a) = show a

instance Pretty Add where
  render (Add e1 e2) = "(" ++ pretty e1 ++ "+" ++ pretty e2 ++ ")"

instance Pretty Mult where
  render (Mult e1 e2) = "(" ++ pretty e1 ++ "*" ++ pretty e2 ++ ")"

instance (Pretty f, Pretty g) => Pretty (f :+: g) where
  render (Inr a) = render a
